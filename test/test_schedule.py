...
def _step(sid, dur, requires=None, stype="cook", **kw):
    return RecipeStep(
        id=sid, raw_text=kw.get("raw_text", sid), label=kw.get("label", sid), type=stype,
        estimated_duration_minutes=dur, requires=requires or [],
...
def test_independent_steps_run_in_parallel():
    ...
    sched = schedule_recipe(_recipe([_step("step-1", 5, stype="prep"),
                                     _step("step-2", 8, stype="prep")]))
    by_id = {s.id: s for s in sched.steps}
...
def test_active_time_excludes_passive_waiting():
    ...
    sched = schedule_recipe(_recipe([
        ...
        _step("step-2", 30, stype="cook", label="Roast veg", raw_text="Roast in the oven for 30 minutes",
              requires=["step-1"]),
    ]))
