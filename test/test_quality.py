...
def _scheduled_step(sid, start, dur, requires=None, raw_text="do a thing"):
    return ScheduledStep(
        id=sid, raw_text=raw_text, label=sid, type="cook", duration_min=dur,
        start_min=start, end_min=start + dur, requires=requires or [],
    )


def test_clean_run_passes_thresholds():
    raw = RawRecipe(id="r1", title="T", method="Preheat oven. Chop the onions finely.")
    extracted = [ExtractedRecipe(id="r1", title="T", cookable=True,
                                 steps=[ExtractedStep(id="step-1", text="Chop the onions finely.")])]
    scheduled = [ScheduledRecipe(id="r1", title="T", total_time_min=5, active_time_min=5,
                                 steps=[_scheduled_step("step-1", 0, 5,
                                                        raw_text="Chop the onions finely.")])]
...
def test_empty_cookable_recipe_flagged():
    ...
    extracted = [ExtractedRecipe(id="r1", title="T", cookable=True,
                                 steps=[ExtractedStep(id="step-1", text="Cook it.")])]
...
def test_timeline_violation_detected():
    ...
    bad = ScheduledStep(id="step-1", raw_text="Chop the onions.", label="x", type="prep",
                        duration_min=5, start_min=0, end_min=99)  # end != start+dur
