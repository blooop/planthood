...
def load_data():
    ...
    with open(data_dir / "recipes_parsed.json", "r") as f:
        parsed = json.load(f)

    with open(data_dir / "recipes_with_schedule.json", "r") as f:
...
def print_analysis(dep_stats, sched_stats):
    ...
    if sched_stats['time_distribution']:
        ...
        min_time = min(t for t in sched_stats['time_distribution'] if t > 0) if any(t > 0 for t in sched_stats['time_distribution']) else 0
    ...
    if sched_stats['critical_path_ratios']:
        avg_critical = sum(sched_stats['critical_path_ratios']) / len(sched_stats['critical_path_ratios'])
